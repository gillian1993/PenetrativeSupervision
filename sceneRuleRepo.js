...
export function validateRuleRecord(rule){
  ...
  else if(evidence.some((item)=>!item.name.trim()||!item.source.trim()||!item.sourceField.trim()))blockers.push({field:'evidence',tab:'output',message:'证据名称、数据来源和来源字段不能为空'})
  const policies=normalizePolicyList(rule.policy_json)
  if(!policies.some((item)=>item.name.trim()))blockers.push({field:'policy',tab:'output',message:'至少配置一条制度依据'})
