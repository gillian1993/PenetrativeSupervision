...
export async function handleEffectSampleApi(req,res,url,{pool,sendJson,readBody}){
  ...
  await pool.query('INSERT INTO rule_effect_samples (id,run_record_id,conclusion,effect_label,feedback_json) VALUES (?,?,?,?,?)',[id,objectId,conclusion,effectLabel,JSON.stringify({objectType:payload.objectType||'预警',summary:payload.summary||'',operator:payload.operator||'尹晨阳',source:'风险处置闭环'})])
