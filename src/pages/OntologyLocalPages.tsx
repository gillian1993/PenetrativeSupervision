...
export function OntologyListPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const remove = async () => {
    ...
    setToast(result.message)
    if (!result.ok) return
    addAudit({ operator: '陈洁', organization: '数据管理部', action: '删除图谱结构草稿', objectType: '图谱结构', objectId: listDeleteTarget.id, summary: `${productText(listDeleteTarget.name)} ${listDeleteTarget.version}`, result: '成功', risk: '高危' })
  ...
  return <>
    ...
    <Modal open={!!target} title={target?.status === '已发布' ? '复制图谱结构新版本' : target?.status === '待校验' ? '发布图谱结构版本' : '校验图谱结构版本'} description={target ? `${productText(target.name)} ${target.version}` : ''} confirmText={target?.status === '已发布' ? '确认复制' : target?.status === '待校验' ? '确认发布' : '开始校验'} onClose={() => setTarget(null)} onConfirm={confirmVersionAction}><div className="publish-checklist">{['基本信息完整','至少定义一个类','属性和关系引用真实类','结构元素与引用完整','发布后版本只读'].map((label, index) => { const pass = index === 1 ? Boolean(target && target.classes > 0) : index === 2 || index === 3 ? Boolean(target && (target.validation?.blockers.length || 0) === 0) : true; return <div key={label}><Icon name={pass ? 'check' : 'warning'}/><span>{label}</span><StatusTag>{pass ? '通过' : '未通过'}</StatusTag></div> })}</div></Modal>
    <Modal open={!!listDeleteTarget} title="删除图谱结构草稿" description={listDeleteTarget ? `${productText(listDeleteTarget.name)} · ${listDeleteTarget.id}` : ''} confirmText="确认删除" danger onClose={() => setListDeleteTarget(null)} onConfirm={remove}><div className="alert-box danger"><Icon name="warning"/><span>仅未发布且未被场景、规则或图谱引用的图谱结构版本可以删除，操作不可恢复。</span></div></Modal>
  </>
}
...
export function OntologyEditorPage() {
  ...
  return <>
    <div className="editor-header"><div><button className="back-button" onClick={() => navigate('/graphs/structures')}>‹ 返回图谱结构</button><p className="eyebrow">图谱结构版本编辑 / {item.id}</p><h1>{productText(item.name)}</h1><div className="editor-meta"><StatusTag>{item.status}</StatusTag><span>版本 {item.version}</span><span>{productText(item.scope)}</span><span>最近保存：{item.updatedAt}</span></div></div><div className="ontology-editor-header-tools"><div className="ontology-editor-mode" aria-label="图谱结构编辑方式"><span>编辑方式</span><div role="group"><button className={editorMode === 'structured' ? 'active' : ''} aria-pressed={editorMode === 'structured'} onClick={() => switchEditorMode('structured')}><Icon name="menu" size={15}/>结构化配置</button><button className={editorMode === 'visual' ? 'active' : ''} aria-pressed={editorMode === 'visual'} onClick={() => switchEditorMode('visual')}><Icon name="graph" size={15}/>可视化建模</button></div></div><div className="page-actions">{readonly ? <Button variant="primary" onClick={copy}>复制新版本</Button> : <><Button onClick={save}>保存草稿</Button><Button onClick={validate}>校验</Button><Button variant="primary" onClick={() => setPublishOpen(true)}>发布</Button></>}</div></div></div>
    ...
    <Drawer open={elementOpen} title={`${editTarget ? '编辑' : '新增'}${typeLabels[elementForm.type]}`} eyebrow="结构元素" onClose={closeElementEditor} footer={<><Button onClick={closeElementEditor}>取消</Button>{editTarget && elementForm.type === 'property' && <Button onClick={() => resetPropertyEditor(elementForm.ownerCode, elementForm.dataType)}>返回新增</Button>}{!editTarget && elementForm.type === 'property' && <Button onClick={() => void saveElement(true)}>保存并继续新增</Button>}<Button variant="primary" onClick={() => void saveElement()}>{editTarget ? '保存修改' : '保存元素'}</Button></>}>
      <div className="form-stack">
        <Field label="元素类型"><input value={typeLabels[elementForm.type]} disabled/></Field>
        <Field label="元素编码 *"><input value={elementForm.code} onChange={(event) => setElementForm({ ...elementForm, code: event.target.value })} placeholder="请输入唯一的元素编码"/></Field>
        <Field label="中文名称 *"><input value={elementForm.name} onChange={(event) => setElementForm({ ...elementForm, name: event.target.value })}/></Field>
        {elementForm.type === 'property' && <Field label="所属类 *"><select value={elementForm.ownerCode || ''} onChange={(event) => changePropertyOwner(event.target.value)}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name} · {option.code}</option>)}</select></Field>}
