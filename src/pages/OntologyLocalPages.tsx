...
const elementInitial = { type: 'class' as OntologyElementType, code: '', name: '', ownerCode: '', targetCode: '', dataType: '实体类型', constraint: '', description: '' }
const typeLabels: Record<OntologyElementType, string> = { class: '实体类型', property: '属性字段', relation: '关系类型' }
...
export function OntologyListPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return <>
    {!embedded && <PageHeader eyebrow="能力中心 / 知识图谱 / 图谱结构" title="图谱结构" description="维护知识图谱中的实体类型、属性字段与关系类型定义。" actions={<><Button icon="refresh" onClick={() => setToast('图谱结构列表已刷新')}>刷新</Button><Button variant="primary" icon="plus" onClick={() => navigate('/ontology/new')}>新建图谱结构</Button></>}/>}
    {embedded && <div className="section-toolbar"><div><strong>图谱结构</strong><span>维护知识图谱中的实体类型、属性字段与关系类型定义。</span></div><div className="page-actions"><Button icon="refresh" onClick={() => setToast('图谱结构列表已刷新')}>刷新</Button><Button variant="primary" icon="plus" onClick={() => navigate('/ontology/new')}>新建图谱结构</Button></div></div>}
...
export function OntologyCreatePage() {
  ...
  const stepCards = [
    ...
    { key: 'class', label: '实体类型定义', description: '创建草稿后继续配置', done: false },
    { key: 'relation', label: '属性字段与关系类型', description: '创建草稿后继续配置', done: false },
  ]
  ...
  return <>
    ...
    <main className="rule-step-content">
      <Panel title="1. 基本信息" subtitle="先创建图谱结构草稿；实体类型、属性字段、关系类型和版本校验在编辑页继续维护">
        ...
        <div className="next-action-card"><Icon name={basicDone ? 'check' : 'clock'}/><div><strong>{basicDone ? '基本信息已完整' : '请先补齐基本信息'}</strong><span>{basicDone ? '点击下一步后创建图谱结构草稿，并进入实体类型定义配置。' : '至少填写结构名称和监管领域；结构编码可留空自动生成，结构说明可后续补充。'}</span></div></div>
      </Panel>
    </main>
    <footer className="editor-action-bar"><div><span className={dirty ? 'dirty-dot' : ''}/><strong>{dirty ? '新建内容尚未保存' : '尚未创建图谱结构草稿'}</strong><small>创建后可继续维护实体类型、属性字段和关系类型</small></div><div><Button onClick={cancel}>取消</Button><Button disabled={saving} onClick={() => void createDraft(false)}>{saving ? '正在创建…' : '保存草稿'}</Button><Button variant="primary" disabled={saving} onClick={() => void createDraft(true)}>{saving ? '正在创建…' : '下一步'}</Button></div></footer>
  </>
}
...
export function OntologyEditorPage() {
  ...
  const saveElement = async (continueAdding = false) => {
    ...
    if (elementForm.type === 'property' && !elementForm.ownerCode) { setToast('请先选择属性字段所属实体类型'); return }
    if (elementForm.type === 'relation' && (!elementForm.ownerCode || !elementForm.targetCode)) { setToast('请先选择关系类型的起点实体类型和终点实体类型'); return }
    const named = (code: string) => classOptions.find((row) => row.code === code)?.name || code
    const payload = { ...elementForm, dataType: elementForm.type === 'class' ? '实体类型' : elementForm.type === 'relation' && elementForm.ownerCode && elementForm.targetCode ? `${named(elementForm.ownerCode)} → ${named(elementForm.targetCode)}` : elementForm.dataType }
  ...
  return <>
    ...
    <Panel className={'editor-panel' + (editorMode === 'visual' ? ' ontology-visual-editor-panel' : '')}>{editorMode === 'structured' && <Tabs value={tab} onChange={setTab} items={[{ key: 'basic', label: '基本信息' }, { key: 'class', label: '实体类型', count: item.classes }, { key: 'property', label: '属性字段', count: item.properties }, { key: 'relation', label: '关系类型', count: item.relations }, { key: 'versions', label: '版本记录' }]}/>}
    ...
    <Drawer open={elementOpen} title={elementDrawerTitle} eyebrow="结构元素" onClose={closeElementEditor} footer={elementDrawerFooter}>
      <div className="form-stack">
        ...
        {elementForm.type === 'property' && <Field label="所属实体类型 *"><select value={elementForm.ownerCode || ''} disabled={readonly} onChange={(event) => changePropertyOwner(event.target.value)}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name} · {option.code}</option>)}</select></Field>}
        {elementForm.type === 'relation' && <><Field label="起点实体类型 *"><select value={elementForm.ownerCode || ''} disabled={readonly} onChange={(event) => updateRelationEndpoint('ownerCode', event.target.value)}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name} · {option.code}</option>)}</select></Field><Field label="终点实体类型 *"><select value={elementForm.targetCode || ''} disabled={readonly} onChange={(event) => updateRelationEndpoint('targetCode', event.target.value)}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name} · {option.code}</option>)}</select></Field><div className="alert-box"><Icon name="link"/><span>可视化建模中连接两个实体类型后，系统会自动带出关系编码、名称和方向；你也可以在保存前手动调整。</span></div></>}
        {elementForm.type !== 'class' && <Field label={elementForm.type === 'relation' ? '关系方向' : '数据类型'}><input value={elementForm.type === 'relation' ? `${classOptions.find((option) => option.code === elementForm.ownerCode)?.name || elementForm.ownerCode || '起点实体类型'} → ${classOptions.find((option) => option.code === elementForm.targetCode)?.name || elementForm.targetCode || '终点实体类型'}` : elementForm.dataType} disabled={readonly || elementForm.type === 'relation'} onChange={(event) => setElementForm({ ...elementForm, dataType: event.target.value })}/></Field>}
      ...
      {elementForm.type === 'property' && <section className="property-manager-list"><header><div><strong>当前实体类型属性字段</strong><span>{classOptions.find((option) => option.code === managedPropertyOwner)?.name || managedPropertyOwner || '未选择所属实体类型'} · {managedPropertyOwner || '—'}</span></div><StatusTag>{managedProperties.length} 条</StatusTag></header>{managedProperties.length ? <div>{managedProperties.map((property) => <article className={editTarget?.id === property.id ? 'editing' : ''} key={property.id}><div><strong>{property.name}</strong><small>{property.code}</small></div><span>{property.dataType || '未设置类型'} · {property.constraint || '无约束'}</span><div className="row-actions"><button onClick={() => openElement('property', property)}>{readonly ? '查看' : '编辑'}</button>{!readonly && <button className="danger-link" onClick={() => setDeleteTarget(property)}>删除</button>}</div></article>)}</div> : <p>当前实体类型尚未配置属性字段，保存后会自动出现在这里。</p>}</section>}
    </Drawer>
    <Modal open={publishOpen} title="发布图谱结构版本" description={`${productText(item.name)} ${item.version}`} confirmText="确认发布" onClose={() => setPublishOpen(false)} onConfirm={publish}><div className="publish-checklist">{[["已完成结构校验",item.status === "待校验"],["至少定义一个实体类型",item.classes > 0],["属性字段和关系类型引用真实实体类型",(item.validation?.blockers.length || 0) === 0],["结构元素与引用完整",(item.validation?.blockers.length || 0) === 0],["发布/下架后版本只读",true]].map(([label, pass]) => <div key={String(label)}><Icon name={pass ? "check" : "warning"}/><span>{label}</span><StatusTag>{pass ? "通过" : "未通过"}</StatusTag></div>)}</div></Modal>
