...
const typeLabels: Record<OntologyElementType, string> = { class: '本体类', property: '属性', relation: '关系', event: '事件' }
...
export function OntologyListPage() {
  ...
  return <>
    <PageHeader eyebrow="本体与图谱 / 本体管理" title="本体管理" description="维护监管基础本体和领域扩展本体，统一类、属性、关系与事件语义。" actions={<><Button icon="refresh" onClick={() => setToast('本体列表已刷新')}>刷新</Button><Button variant="primary" icon="plus" onClick={() => setCreateOpen(true)}>新建本体</Button></>}/>
...
export function OntologyEditorPage() {
  ...
  const activeType = (['class','property','relation','event'].includes(tab) ? tab : 'class') as OntologyElementType
  ...
  const openElement = (type: OntologyElementType, element?: OntologyElement) => {
    setEditTarget(element || null)
    const firstClass = classOptions[0]
    setElementForm(element ? { ...element, ownerCode: element.ownerCode || '', targetCode: element.targetCode || '' } : { ...elementInitial, type, ownerCode: type === 'class' ? '' : firstClass?.code || '', targetCode: type === 'relation' ? firstClass?.code || '' : '', dataType: type === 'class' ? '本体类' : type === 'property' ? '文本' : type === 'relation' ? '主体 → 主体' : '业务事件' })
    setElementOpen(true)
  }
  const saveElement = async () => {
    const named = (code: string) => classOptions.find((row) => row.code === code)?.name || code
    const payload = { ...elementForm, dataType: elementForm.type === 'relation' && elementForm.ownerCode && elementForm.targetCode ? `${named(elementForm.ownerCode)} → ${named(elementForm.targetCode)}` : elementForm.dataType }
  ...
  return <>
    ...
    <Drawer open={elementOpen} title={`${editTarget ? '编辑' : '新增'}${typeLabels[elementForm.type]}`} eyebrow="本体元素" onClose={() => { setElementOpen(false); setEditTarget(null) }} footer={<><Button onClick={() => { setElementOpen(false); setEditTarget(null) }}>取消</Button><Button variant="primary" onClick={saveElement}>保存元素</Button></>}>
      <div className="form-stack">
        ...
        {elementForm.type === 'property' && <Field label="所属对象 *"><select value={elementForm.ownerCode || ''} onChange={(event) => setElementForm({ ...elementForm, ownerCode: event.target.value })}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name}</option>)}</select></Field>}
        {elementForm.type === 'relation' && <><Field label="起点对象 *"><select value={elementForm.ownerCode || ''} onChange={(event) => setElementForm({ ...elementForm, ownerCode: event.target.value })}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name}</option>)}</select></Field><Field label="终点对象 *"><select value={elementForm.targetCode || ''} onChange={(event) => setElementForm({ ...elementForm, targetCode: event.target.value })}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name}</option>)}</select></Field></>}
        {elementForm.type === 'event' && <Field label="事件主体 *"><select value={elementForm.ownerCode || ''} onChange={(event) => setElementForm({ ...elementForm, ownerCode: event.target.value })}>{classOptions.map((option) => <option value={option.code} key={option.code}>{option.name}</option>)}</select></Field>}
        <Field label={elementForm.type === 'relation' ? '关系方向' : '数据类型'}><input value={elementForm.type === 'relation' ? `${classOptions.find((option) => option.code === elementForm.ownerCode)?.name || elementForm.ownerCode || '起点对象'} → ${classOptions.find((option) => option.code === elementForm.targetCode)?.name || elementForm.targetCode || '终点对象'}` : elementForm.dataType} disabled={elementForm.type === 'relation'} onChange={(event) => setElementForm({ ...elementForm, dataType: event.target.value })}/></Field>
        <Field label="约束"><input value={elementForm.constraint} onChange={(event) => setElementForm({ ...elementForm, constraint: event.target.value })} placeholder="主标识、必填、多对多、目标事件等"/></Field>
    ...
    <Modal open={publishOpen} title="发布本体版本" description={`${item.name} ${item.version}`} confirmText="确认发布" onClose={() => setPublishOpen(false)} onConfirm={publish}><div className="publish-checklist">{[["已完成本体校验",item.status === "待校验"],["至少定义一个本体类",item.classes > 0],["属性、关系与事件引用真实对象",(item.validation?.blockers.length || 0) === 0],["发布后版本只读",true]].map(([label, pass]) => <div key={String(label)}><Icon name={pass ? "check" : "warning"}/><span>{label}</span><StatusTag>{pass ? "通过" : "未通过"}</StatusTag></div>)}</div></Modal>
