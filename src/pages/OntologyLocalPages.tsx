...
export function OntologyEditorPage() {
  ...
  const openElement = (type: OntologyElementType, element?: OntologyElement, defaults: Partial<typeof elementInitial> = {}, position?: CanvasPosition) => {
    ...
    setElementForm(element ? { ...elementInitial, ...element, ownerCode: element.ownerCode || '', targetCode: element.targetCode || '' } : { ...elementInitial, type, code: type === 'property' && ownerCode ? `${ownerCode}.` : '', ownerCode, targetCode: type === 'relation' ? firstClass?.code || '' : '', dataType: type === 'class' ? '本体类' : type === 'property' ? '文本' : '本体类 → 本体类', ...defaults })
    setElementOpen(true)
  }
  ...
  return <>
    ...
    <Modal open={publishOpen} title="发布本体版本" description={`${item.name} ${item.version}`} confirmText="确认发布" onClose={() => setPublishOpen(false)} onConfirm={publish}><div className="publish-checklist">{[["已完成本体校验",item.status === "待校验"],["至少定义一个本体类",item.classes > 0],["属性和关系引用真实本体类",(item.validation?.blockers.length || 0) === 0],["本体元素与引用完整",(item.validation?.blockers.length || 0) === 0],["发布后版本只读",true]].map(([label, pass]) => <div key={String(label)}><Icon name={pass ? "check" : "warning"}/><span>{label}</span><StatusTag>{pass ? "通过" : "未通过"}</StatusTag></div>)}</div></Modal>
