...
export function OntologyEditorPage() {
  ...
  const openElement = (type: OntologyElementType, element?: OntologyElement, defaults: Partial<typeof elementInitial> = {}, position?: CanvasPosition) => {
    ...
    setElementForm(element ? { ...element, ownerCode: element.ownerCode || '', targetCode: element.targetCode || '' } : { ...elementInitial, type, ownerCode: type === 'class' ? '' : firstClass?.code || '', targetCode: type === 'relation' ? firstClass?.code || '' : '', dataType: type === 'class' ? '本体类' : type === 'property' ? '文本' : type === 'relation' ? '主体 → 主体' : '业务事件', ...defaults })
    setElementOpen(true)
  }
  const saveElement = async () => {
    ...
    setToast(result.message)
    if (!result.ok) return
    if (!editTarget && pendingCanvasPosition && (elementForm.type === 'class' || elementForm.type === 'event')) saveOntologyLayoutPosition(item.id, elementForm.code, pendingCanvasPosition)
  ...
  return <>
    ...
    <Panel className={'editor-panel' + (editorMode === 'visual' ? ' ontology-visual-editor-panel' : '')}>{editorMode === 'structured' && <Tabs value={tab} onChange={setTab} items={[{ key: 'basic', label: '基本信息' }, { key: 'class', label: '本体类', count: item.classes }, { key: 'property', label: '属性', count: item.properties }, { key: 'relation', label: '关系', count: item.relations }, { key: 'event', label: '事件', count: item.events }, { key: 'versions', label: '版本记录' }]}/>}
