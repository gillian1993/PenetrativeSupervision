...
type WarningAction = 'assign' | 'observe' | 'release' | 'escalate' | 'feedback' | 'review' | 'compensate' | null
...
const actionMeta: Record<Exclude<WarningAction, null>, { title: string; description: string; confirm: string; danger?: boolean }> = {
  ...
  observe: { title: '转为持续观察', description: '暂不确认风险，记录观察原因和下次关注时间。', confirm: '确认观察' },
  release: { title: '解除预警', description: '必须填写解除原因并引用证据，提交后生成规则效果样本。', confirm: '确认解除', danger: true },
  escalate: { title: '升级风险事件', description: '创建唯一主风险事件并固化初始证据、版本与责任信息。', confirm: '确认升级', danger: true },
...
export function WarningListPage() {
  ...
  const observeWarning = useAppStore((state) => state.observeWarning)
  ...
  const [draft, setDraft] = useState({ keyword: '', status: searchParams.get('status') === 'open' ? '待处理' : '全部', stage: '全部', level: searchParams.get('level') === 'high' ? '重大、高' : '全部', scene: '全部', org: searchParams.get('org') || '全部', evidence: '全部' })
  ...
  const [form, setForm] = useState({ owner: '李华', dueAt: '2026-07-18 18:00', requirement: '核查主体关联事实、业务审批过程和相关证明材料。', reason: '', evidence: '', followAt: '2026-07-20 09:00' })
  ...
  const openAction = (item: Warning, next: WarningAction) => {
    setTarget(item)
    setAction(next)
    setForm({ owner: next === 'assign' ? '李华' : (item.owner || '李华'), dueAt: '2026-07-18 18:00', requirement: '核查主体关联事实、业务审批过程和相关证明材料。', reason: '', evidence: '', followAt: '2026-07-20 09:00' })
  }
  const executeAction = () => {
    ...
    if (action === 'assign') {
      ...
      const result = observeWarning(target.id, form.reason, form.followAt); setToast(result.message); if (result.ok) setAction(null)
      ...
      if (form.reason.trim().length < 8) { setToast('请填写完整的风险确认说明'); return }
      const eventId = escalateWarning(target.id, form.reason); setToast(`已升级为风险事件 ${eventId}`); setAction(null)
  ...
  return <>
    <PageHeader eyebrow="风险监管 / 统一预警" title="统一预警" description="统一查询事前、事中和事后预警，开展证据研判、核查、观察、解除和风险升级。" actions={<><span className="updated-time">来源：{searchParams.get('source') || '菜单查询'} · 当前角色：{currentRole}</span><Button icon="refresh" onClick={query}>刷新</Button></>}/>
    <FilterGrid onReset={() => { const value = { keyword: '', status: '全部', stage: '全部', level: '全部', scene: '全部', org: '全部', evidence: '全部' }; setDraft(value); setFilters(value); setPage(1) }} onSearch={query}>
      ...
      <Field label="处理状态"><select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value })}><option>全部</option><option>待处理</option><option>核查中</option><option>待复核</option><option>持续观察</option><option>已解除</option><option>已升级</option></select></Field>
...
function WarningActionForm({ action, warning, form, onChange }: { action: Exclude<WarningAction, null>; warning: Warning; form: { owner: string; dueAt: string; requirement: string; reason: string; evidence: string; followAt: string }; onChange: (value: typeof form) => void }) {
  return <div className="form-stack"><div className="operation-target"><RiskTag level={warning.level}/><div><strong>{warning.title}</strong><span>{warning.id} · {warning.target}</span></div></div>{action === 'assign' && <><Field label="责任人 *"><select value={form.owner} onChange={(event) => onChange({ ...form, owner: event.target.value })}><option>李华</option><option>王宁</option><option>周航</option><option>孙凯</option></select></Field><Field label="完成时限 *"><input value={form.dueAt} onChange={(event) => onChange({ ...form, dueAt: event.target.value })}/></Field><Field label="核查要求 *"><textarea value={form.requirement} onChange={(event) => onChange({ ...form, requirement: event.target.value })}/></Field></>}{action === 'observe' && <><Field label="观察原因 *"><textarea value={form.reason} onChange={(event) => onChange({ ...form, reason: event.target.value })}/></Field><Field label="下次关注时间 *"><input value={form.followAt} onChange={(event) => onChange({ ...form, followAt: event.target.value })}/></Field></>}{action === 'release' && <><Field label="解除原因 *"><select value={form.reason} onChange={(event) => onChange({ ...form, reason: event.target.value })}><option value="">请选择</option><option>规则误报</option><option>条件已经消失</option><option>已取得有效授权</option></select></Field><Field label="引用证据 *"><input value={form.evidence} onChange={(event) => onChange({ ...form, evidence: event.target.value })} placeholder="输入证据编号或说明"/></Field></>}{action === 'escalate' && <><div className="alert-box danger"><Icon name="warning"/><span>升级后预警状态变为已升级，初始证据快照不可覆盖。</span></div><Field label="风险确认说明 *"><textarea value={form.reason} onChange={(event) => onChange({ ...form, reason: event.target.value })}/></Field></>}{action === 'compensate' && <div className="alert-box"><Icon name="refresh"/><span>补偿任务将使用预警生成时记录的本体、图谱和规则版本重新固化缺失证据。</span></div>}</div>
}
...
export function WarningDetailPage() {
  ...
  const [form, setForm] = useState({ owner: '李华', dueAt: '2026-07-18 18:00', requirement: '核查主体关联事实、业务审批过程和相关证明材料。', reason: '', evidence: '', followAt: '2026-07-20 09:00', conclusion: '风险存在', facts: '', materials: [] as string[], reviewResult: '退回补充' as '退回补充' | '持续观察' | '解除预警' | '升级风险事件' })
  if (!warning) return <EmptyState title="预警不存在或无权访问" description="请返回预警列表重新选择可访问对象。"/>
  const openAction = (next: WarningAction) => { setAction(next); setForm({ ...form, owner: next === 'assign' ? '李华' : (warning.owner || '李华'), reason: '', evidence: '', facts: '', materials: [] }) }
  const execute = () => {
    if (!action) return
    ...
    if (action === 'observe') { const result = observeWarning(warning.id, form.reason, form.followAt); setToast(result.message); if (result.ok) setAction(null) }
    ...
    if (action === 'escalate') { if (form.reason.trim().length < 8) { setToast('请填写完整风险确认说明'); return }; const eventId = escalateWarning(warning.id, form.reason); setAction(null); navigate(`/risk/events/${eventId}`) }
    ...
    if (action === 'review') { const result = reviewWarning(warning.id, form.reviewResult, form.reason); setToast(result.message); if (result.ok) { setAction(null); if (result.objectId) navigate(`/risk/events/${result.objectId}`) } }
  }
...
function DetailActionForm({ action, warning, form, onChange }: { action: Exclude<WarningAction, null>; warning: Warning; form: { owner: string; dueAt: string; requirement: string; reason: string; evidence: string; followAt: string; conclusion: string; facts: string; materials: string[]; reviewResult: '退回补充' | '持续观察' | '解除预警' | '升级风险事件' }; onChange: (value: typeof form) => void }) {
  if (['assign','observe','release','escalate','compensate'].includes(action)) return <WarningActionForm action={action} warning={warning} form={form} onChange={(value) => onChange({ ...form, ...value })}/>
  ...
  return <div className="form-stack"><Field label="复核动作 *"><select value={form.reviewResult} onChange={(event) => onChange({ ...form, reviewResult: event.target.value as typeof form.reviewResult })}><option>退回补充</option><option>持续观察</option><option>解除预警</option><option>升级风险事件</option></select></Field><Field label="复核说明 *"><textarea value={form.reason} onChange={(event) => onChange({ ...form, reason: event.target.value })} placeholder="说明判断依据、缺失项或风险确认事实"/></Field>{form.reviewResult === '解除预警' && <Field label="引用证据"><input value={form.evidence} onChange={(event) => onChange({ ...form, evidence: event.target.value })} placeholder="核查反馈与补充材料"/></Field>}</div>
}
...
export function RiskEventDetailPage() {
  ...
  return <>
    ...
    <Panel className="risk-event-detail"><Tabs value={tab} onChange={setTab} items={[{ key: 'facts', label: '风险事实' }, { key: 'evidence', label: '证据材料', count: 8 }, { key: 'timeline', label: '处理时间线', count: 5 }, { key: 'rectification', label: '整改反馈', count: event.status === '待复核' || event.status === '已关闭' ? 2 : 0 }, { key: 'review', label: '复核记录', count: event.status === '已关闭' ? 2 : 1 }]}/>{tab === 'facts' && <RiskFacts event={event}/>} {tab === 'evidence' && <EvidenceTable filter="全部" onFilter={() => undefined} onSelect={() => setToast('证据预览已打开')}/>} {tab === 'timeline' && <RiskTimeline event={event}/>} {tab === 'rectification' && <RectificationView event={event}/>} {tab === 'review' && <ReviewView event={event}/>}</Panel>
