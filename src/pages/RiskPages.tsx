...
function EvidenceTable({ filter, onFilter, onSelect, rows }: { filter: string; onFilter: (value: string) => void; onSelect: (key: string) => void; rows?: DemoEvidence[] }) {
  ...
  if (rows) {
    const visible = normalizedQuery ? rows.filter((row) => [row.id, row.description, row.type, row.fieldOrRelation, row.value, row.sourceSystem, row.sourceRecord, row.validity, row.collectedAt].filter(Boolean).join(' ').toLowerCase().includes(normalizedQuery)) : rows
    return <div className="evidence-list-view"><div className="evidence-filter"><div className="evidence-search-box"><span>证据搜索</span><input type="search" value={query} onChange={(event) => onFilter(event.target.value)} placeholder="搜索证据编号、结论、字段、来源或证据值"/></div><div className="evidence-count-badge">数据库证据 <strong>{visible.length}/{rows.length}</strong> 项</div></div><div className="table-container evidence-table"><table><thead><tr><th>证据编号 / 结论</th><th>类型</th><th>字段 / 关系</th><th>证据值</th><th>来源</th><th>有效性</th><th>采集时间</th></tr></thead><tbody>{visible.map((row) => <tr key={row.id}><td><button className="table-link title-cell" onClick={() => onSelect(row.sourceRecord)}><strong>{row.description || row.type}</strong><span>{row.id}</span></button></td><td>{row.type}</td><td>{row.fieldOrRelation}</td><td>{evidenceDisplayValue(row)}</td><td>{row.sourceSystem}<small className="cell-sub">{row.sourceRecord}</small></td><td><StatusTag>{row.validity}</StatusTag></td><td>{row.collectedAt}</td></tr>)}</tbody></table></div>{!visible.length&&<EmptyState title="没有匹配的实际证据" description="可按证据编号、名称、字段、来源记录或证据值继续搜索。"/>}</div>
  }
  ...
  const visible = normalizedQuery ? allRows.filter((row) => row.join(' ').toLowerCase().includes(normalizedQuery)) : allRows
  return <div className="evidence-list-view"><div className="evidence-filter"><div className="evidence-search-box"><span>证据搜索</span><input type="search" value={query} onChange={(event) => onFilter(event.target.value)} placeholder="搜索证据编号、名称、对象、来源或路径"/></div><div className="evidence-count-badge">共 <strong>{visible.length}/{allRows.length}</strong> 项证据</div></div><div className="table-container evidence-table"><table><thead><tr><th>证据编号 / 名称</th><th>类型</th><th>关联对象</th><th>来源</th><th>状态</th><th>密级</th><th>操作</th></tr></thead><tbody>{visible.map((row) => <tr key={row[0]}><td><strong>{row[1]}</strong><small className="cell-sub">{row[0]} · {row[8]}</small></td><td>{row[2]}</td><td>{row[3]}</td><td>{row[4]}</td><td><StatusTag>{row[5]}</StatusTag></td><td><StatusTag>{row[6]}</StatusTag></td><td><div className="row-actions"><button onClick={() => onSelect(row[7])}>预览</button><button onClick={() => onSelect(row[7])}>定位</button></div></td></tr>)}</tbody></table></div>{!visible.length&&<EmptyState title="没有匹配的实际证据" description="可按证据编号、名称、对象、来源或路径继续搜索。"/>}</div>
}
