...
function DatabaseGraphView({ detail, filter, viewMode, showEvents, selected, onSelect }: { detail: DemoWarningDetail; filter: DatabaseGraphRelationFilter; viewMode: DatabaseGraphViewMode; showEvents: boolean; selected: string; onSelect: (id: string) => void }) {
  ...
  return <div className={`database-graph-view ontology-instance-view ${riskChain ? 'risk-chain-view' : 'full-ontology-view'} ${selected ? 'has-selection' : ''} ${spacious ? 'spacious' : dense ? 'dense' : ''}`}>
    <div className="database-graph-header"><div><strong>本体实例证据图 · {detail.graph.id}</strong><span>{detail.ontology?.name || detail.ontology?.id || '监管本体'} {detail.ontology?.version || ''} · {detail.warning.caseId} · {layout.points.length}/{detail.graph.nodes.length} 个节点实例、{layout.relations.length}/{detail.graph.relations.length} 个关系实例、{showEvents ? eventLayout.points.length : 0}/{detail.graph.events.length} 个事件实例</span></div><StatusTag>{viewMode}</StatusTag></div>
    ...
    <svg viewBox={`0 0 ${layout.width} ${canvasHeight}`} role="img" aria-label={`基于本体实例化的MySQL预警证据子图，当前模式${filter}`}>
      ...
      {layout.points.map((point) => {
        ...
        return <g key={point.node.id} role="button" tabIndex={0} aria-label={`${className}节点实例：${point.node.name}`} className={`database-graph-node ontology-class-instance ${point.target ? 'target' : ''} ${selected === point.node.id ? 'selected' : ''} ${related ? 'related' : ''} ${muted ? 'muted' : ''}`} transform={`translate(${point.x},${point.y})`} onClick={() => onSelect(point.node.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelect(point.node.id) }}>
          ...
          <text className="kind" textAnchor="middle" y="-10">{point.target ? `核心类 · ${classLabel}` : `本体类 · ${classLabel}`}</text>
      ...
      {showEvents && eventLayout.points.map((point) => {
        ...
        return <g key={point.event.id} role="button" tabIndex={0} aria-label={`事件实例：${eventName} · ${point.event.name}`} className={`database-graph-event ${point.event.status === '异常' ? 'danger' : ''} ${selected === point.event.id ? 'selected' : ''} ${muted ? 'muted' : ''}`} transform={`translate(${point.x},${point.y})`} onClick={() => onSelect(point.event.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelect(point.event.id) }}>
          <title>{point.event.name} · 本体事件：{eventName}（{point.event.eventCode}）· 发生时间：{point.event.eventTime} · {point.event.id}</title>
    ...
    <div className="database-graph-legend ontology"><span title="蓝色球形节点为本体类对应的业务节点"><i className="ontology-class"/>节点实例 <b>{layout.points.length}</b></span><span title="节点右上角数字表示属性数量，点击节点查看属性详情"><i className="ontology-property">3</i>节点属性 <b>{mappedPropertyCount}</b></span><span title="带方向箭头的连线表示关系，点击连线查看详情"><i className="ontology-relation"/>关系连线 <b>{edgeGroups.length}</b></span><span title="紫色节点表示业务事件实例，包含发生时间"><i className="ontology-event"/>事件实例 <b>{showEvents ? eventLayout.points.length : `${eventLayout.points.length}（隐藏）`}</b></span>{extensionPropertyCount > 0 && <span className="ontology-extension" title="来源系统存在、但当前本体尚未定义的字段">扩展字段 {extensionPropertyCount}</span>}</div>
...
function DatabaseEventDetail({ event, detail }: { event: DatabaseGraphEvent; detail: DemoWarningDetail }) {
  ...
  return <><KeyValue items={[{ label: '节点实例ID', value: event.id }, { label: '所属本体类', value: `${definition?.name || event.eventCode}（${event.eventCode}）` }, { label: '节点名称', value: event.name }, { label: '发生时间', value: event.eventTime }, { label: '参与对象', value: objectNode ? `${ontologyElementName(detail, 'class', objectNode.classCode)} / ${objectNode.name}` : event.objectId }, { label: '参与主体', value: actorNode?.name || event.actorId || '—' }, { label: '所属组织', value: organizationNode?.name || event.organizationId || '—' }, { label: '涉及金额', value: event.amount === null ? '—' : `${event.amount.toLocaleString()}元` }, { label: '状态', value: <StatusTag>{event.status}</StatusTag> }, { label: '来源系统', value: event.sourceSystem }]}/><Panel title="发生信息" className="drawer-section"><KeyValue items={[{ label: '本体类编码', value: definition?.code || event.eventCode }, { label: '发生时间', value: event.eventTime }, { label: '关联对象所属类', value: objectNode?.classCode || '—' }, { label: '约束', value: definition?.constraint || '—' }, { label: '业务说明', value: definition?.description || '—' }]}/></Panel><DatabaseRawProperties title="节点扩展属性" properties={event.properties}/></>
}
