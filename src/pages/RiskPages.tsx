...
export function WarningDetailPage() {
  ...
  return <div className={`warning-detail-page ${fullScreen ? 'evidence-fullscreen' : ''}`}>
    ...
    <section className="summary-strip warning-summary"><div><span>预警阶段</span><StatusTag>{warning.stage}</StatusTag></div><div><span>处理状态</span><StatusTag>{warning.status}</StatusTag></div><div><span>风险等级</span><RiskTag level={warning.level}/></div><div><span>生成时间</span><strong>{warning.generatedAt}</strong></div><div><span>图谱</span><strong>{databaseDetail?.ontology?.name || databaseDetail?.ontology?.id || (detailLoading ? '加载中' : '暂无图谱结构')}</strong><small>{databaseDetail?.ontology?.version ? `版本 ${databaseDetail.ontology.version}` : detailLoading ? '正在读取预警证据快照' : '未提供版本号'}</small></div></section>
...
function EvidenceTable({ filter, onFilter, onSelect, rows }: { filter: string; onFilter: (value: string) => void; onSelect: (key: string) => void; rows?: DemoEvidence[] }) {
  const query = filter === '全部' ? '' : filter.trim()
  const normalizedQuery = query.toLowerCase()
  if (rows) {
    const visible = normalizedQuery ? rows.filter((row) => [row.description, row.type, row.fieldOrRelation, row.value, row.sourceSystem, row.sourceRecord, row.collectedAt].filter(Boolean).join(' ').toLowerCase().includes(normalizedQuery)) : rows
    return <div className="evidence-list-view"><div className="evidence-filter"><div className="evidence-search-box"><span>证据搜索</span><input type="search" value={query} onChange={(event) => onFilter(event.target.value)} placeholder="搜索证据名称、字段、来源或证据值"/></div><div className="evidence-count-badge">数据库证据 <strong>{visible.length}/{rows.length}</strong> 项</div></div><div className="table-container evidence-table"><table><thead><tr><th>证据名称</th><th>类型</th><th>字段 / 关系</th><th>证据值</th><th>来源</th><th>采集时间</th></tr></thead><tbody>{visible.map((row) => <tr key={row.id}><td><button className="table-link title-cell" onClick={() => onSelect(row.sourceRecord)}><strong>{row.description || row.type}</strong></button></td><td>{row.type}</td><td>{row.fieldOrRelation}</td><td>{evidenceDisplayValue(row)}</td><td>{row.sourceSystem}<small className="cell-sub">{row.sourceRecord}</small></td><td>{row.collectedAt}</td></tr>)}</tbody></table></div>{!visible.length&&<EmptyState title="没有匹配的实际证据" description="可按证据名称、字段、来源记录或证据值继续搜索。"/>}</div>
  }
  ...
  const visible = normalizedQuery ? allRows.filter((row) => [row[1], row[2], row[3], row[4], row[8]].filter(Boolean).join(' ').toLowerCase().includes(normalizedQuery)) : allRows
  return <div className="evidence-list-view"><div className="evidence-filter"><div className="evidence-search-box"><span>证据搜索</span><input type="search" value={query} onChange={(event) => onFilter(event.target.value)} placeholder="搜索证据名称、对象、来源或路径"/></div><div className="evidence-count-badge">共 <strong>{visible.length}/{allRows.length}</strong> 项证据</div></div><div className="table-container evidence-table"><table><thead><tr><th>证据名称</th><th>类型</th><th>关联对象</th><th>来源</th><th>状态</th><th>密级</th><th>操作</th></tr></thead><tbody>{visible.map((row) => <tr key={row[0]}><td><strong>{row[1]}</strong></td><td>{row[2]}</td><td>{row[3]}</td><td>{row[4]}</td><td><StatusTag>{row[5]}</StatusTag></td><td><StatusTag>{row[6]}</StatusTag></td><td><div className="row-actions"><button onClick={() => onSelect(row[7])}>预览</button><button onClick={() => onSelect(row[7])}>定位</button></div></td></tr>)}</tbody></table></div>{!visible.length&&<EmptyState title="没有匹配的实际证据" description="可按证据名称、对象、来源或路径继续搜索。"/>}</div>
}
...
function DatabaseNodeDetail({ node, detail }: { node: DemoWarningDetail['graph']['nodes'][number]; detail: DemoWarningDetail }) {
  ...
  return <>
    <KeyValue items={[{ label: '元素类型', value: '类' }, { label: '元素编码', value: node.classCode }, { label: '中文名称', value: classDefinition?.name || node.name }, { label: '来源系统', value: node.sourceSystem }]}/>
...
export function RiskEventDetailPage() {
  ...
  return <div className={`risk-event-detail-page ${graphFullScreen ? 'graph-fullscreen' : ''}`}>
    ...
    <section className="summary-strip risk-summary"><div><span>风险等级</span><RiskTag level={event.level}/></div><div><span>当前状态</span><StatusTag>{event.status}</StatusTag></div><div><span>来源预警</span><button className="table-link" onClick={() => openSourceWarning(event.warningId)}>{event.warningId}</button></div><div><span>当前处理人</span><strong>{event.owner || '待分配'}</strong></div><div><span>截止时间</span><strong className={event.overdue ? 'danger-text' : ''}>{event.dueAt}</strong><small>{event.overdue ? '已逾期' : '正常'}</small></div><div><span>图谱</span><strong>{sourceDetail?.ontology?.name || sourceDetail?.ontology?.id || (sourceLoading ? '加载中' : '暂无图谱结构')}</strong><small>{sourceDetail?.ontology?.version ? `版本 ${sourceDetail.ontology.version}` : sourceLoading ? '正在读取来源预警快照' : '未提供版本号'}</small></div></section>
...
function RiskFacts({ event, sourceDetail }: { event: RiskEvent; sourceDetail?: DemoWarningDetail | null }) {
  const requirement = event.requirement?.trim() || `请围绕“${event.target}”核实相关风险事实，明确责任边界，并在整改完成后提交处理说明。`
  const coreProjection = sourceDetail ? createDatabaseGraphProjection(sourceDetail, '核心证据链') : null
  const coreGroups = coreProjection ? groupDatabaseGraphRelations(coreProjection.relations) : []
  const corePathSegments = sourceDetail ? coreGroups.slice(0, 3).map((group) => {
    const fromNode = sourceDetail.graph.nodes.find((item) => item.id === group.fromId)
    const toNode = sourceDetail.graph.nodes.find((item) => item.id === group.toId)
    return `${fromNode?.name || group.fromId} —${databaseRelationLabel(group, sourceDetail)}→ ${toNode?.name || group.toId}`
  }) : []
  const corePathText = corePathSegments.length ? corePathSegments.join('；') : event.target
  return <div className="detail-grid">
    <Panel title="风险事实摘要"><div className="fact-callout"><Icon name="warning" size={24}/><div><strong>{event.title}</strong><p>以下命中规则和核心路径来自预警升级时固化的初始证据快照，后续处置不会覆盖。</p></div></div><div className="risk-fact-block"><p className="section-kicker">命中规则{sourceDetail?.runs.length ? ` · ${sourceDetail.runs.length}条` : ''}</p>{sourceDetail?.runs.length ? <div className="rule-hit-list">{sourceDetail.runs.map((run) => <div className="rule-hit" key={run.id}><span>{run.ruleCode}</span><strong>{run.ruleName}</strong><p>{run.actualValueSummary || run.outcome || '该规则已命中并形成风险事实。'}</p></div>)}</div> : <p className="drawer-note">来源预警的命中规则快照暂未加载。</p>}</div><div className="risk-fact-block"><p className="section-kicker">核心路径</p><div className="path-card active"><b>核心证据链</b><span>{corePathText}</span><small>{coreProjection ? `${coreProjection.nodes.length}个对象 · ${coreGroups.length}段关系${coreGroups.length > 3 ? ' · 当前摘要展示前3段' : ''}` : '来源预警证据快照暂未加载'}</small></div></div></Panel>
