...
export const sceneRuleApi={
  ...
  copyScene:(id:string,payload?:{version?:string})=>request<SceneItem>(`/api/scenes/${encodeURIComponent(id)}/copy`,{method:'POST',body:payload?JSON.stringify(payload):undefined}),
  ...
  createRule:(payload:{name:string;code?:string;version?:string;type?:string;stage?:WarningStage;level?:string;domain?:string;libraryName?:string;directoryName?:string;description?:string;status?:string;objectCode?:string;objectName?:string;eventCode?:string;eventName?:string;ontologyId?:string;graphVersion?:string})=>request<RuleItem>('/api/rules',{method:'POST',body:JSON.stringify(payload)}),
  ...
  deleteRuleFromScene:(sceneId:string,ruleVersionId:string)=>request<SceneItem>(`/api/scenes/${encodeURIComponent(sceneId)}/rules/${encodeURIComponent(ruleVersionId)}/asset`,{method:'DELETE'}),
