...
async function validateMappingsForOntology(pool,id,ontologyId){
  ...
  const [invalid]=await pool.query("SELECT sm.id FROM semantic_mappings sm LEFT JOIN ontology_elements oe ON oe.ontology_id=sm.ontology_id AND oe.code=sm.target_code WHERE sm.source_id=? AND sm.ontology_id=? AND (oe.element_id IS NULL OR (sm.mapping_type='节点实例' AND oe.element_type NOT IN ('class','property')) OR (sm.mapping_type='关系' AND oe.element_type<>'relation') OR sm.mapping_type NOT IN ('节点实例','关系'))",[id,ontologyId]);if(invalid.length)throw Object.assign(new Error(`存在${invalid.length}条映射目标或类型无效，请先修正`),{status:409})
