...
async function seedOntologyMappings(pool,sourceId,ontologyId='ONT-PROC'){
  ...
  const classes=codes('class');const properties=codes('property');const relations=codes('relation');const occurrenceClass=classes.find((code)=>code==='PROC.BidConfirmed')||classes[0]
  const rows=[
    ...
    ['节点实例','event_id + occurred_at + participant_id','节点标识、发生时间与关联对象',occurrenceClass||'',99,'待校验'],
  ].filter((row)=>row[3])
...
async function validateMappings(pool,id){const [rows]=await pool.query("SELECT COUNT(*) AS total FROM semantic_mappings WHERE source_id=? AND mapping_type='节点实例'",[id]);if(!Number(rows[0].total))throw Object.assign(new Error('暂无可校验字段映射'),{status:409});await pool.query("UPDATE semantic_mappings SET status='有效' WHERE source_id=? AND mapping_type='节点实例'",[id]);return{ok:true,message:'字段映射校验通过'}}
...
async function updateMappingForOntology(pool,sourceId,mappingId,payload){
  ...
  const target=targets[0];const matches=rows[0].mapping_type==='节点实例'?['class','property'].includes(target.element_type):rows[0].mapping_type==='关系'&&target.element_type==='relation'
  if(!matches)throw Object.assign(new Error(`目标元素类型与${rows[0].mapping_type}映射不匹配`),{status:409})
...
async function validateMappingsForOntology(pool,id,ontologyId){
  await getPublishedOntology(pool,ontologyId)
  const [rows]=await pool.query("SELECT COUNT(*) AS total FROM semantic_mappings WHERE source_id=? AND ontology_id=? AND mapping_type='节点实例'",[id,ontologyId]);if(!Number(rows[0].total))throw Object.assign(new Error('当前数据源与图谱结构暂无可校验字段映射'),{status:409})
  const [invalid]=await pool.query("SELECT sm.id FROM semantic_mappings sm LEFT JOIN ontology_elements oe ON oe.ontology_id=sm.ontology_id AND oe.code=sm.target_code WHERE sm.source_id=? AND sm.ontology_id=? AND sm.mapping_type='节点实例' AND (oe.element_id IS NULL OR oe.element_type NOT IN ('class','property'))",[id,ontologyId]);if(invalid.length)throw Object.assign(new Error(`存在${invalid.length}条字段映射目标无效，请先修正`),{status:409})
  await ensureMappingSet(pool,id,ontologyId)
  await pool.query("UPDATE semantic_mappings SET status='有效' WHERE source_id=? AND ontology_id=? AND mapping_type='节点实例'",[id,ontologyId])
...
async function assessSelectedSources(connection,ontologyId,sourceIds){
  ...
  for(const sourceId of uniqueIds){const source=sourceMap.get(sourceId);const set=await ensureMappingSet(connection,sourceId,ontologyId);const [counts]=await connection.query("SELECT COUNT(*) AS total,SUM(status='有效') AS valid_total FROM semantic_mappings WHERE source_id=? AND ontology_id=? AND mapping_type='节点实例'",[sourceId,ontologyId]);const [invalidRows]=await connection.query("SELECT COUNT(*) AS total FROM semantic_mappings sm LEFT JOIN ontology_elements oe ON oe.ontology_id=sm.ontology_id AND oe.code=sm.target_code WHERE sm.source_id=? AND sm.ontology_id=? AND sm.mapping_type='节点实例' AND (oe.element_id IS NULL OR oe.element_type NOT IN ('class','property'))",[sourceId,ontologyId]);const total=Number(counts[0].total);const valid=Number(counts[0].valid_total||0);const invalid=Number(invalidRows[0].total);let status='ready';let message=`字段映射 R${set.revision} 已就绪`;if(source.status!=='启用'){status='disabled';message=`数据源 ${source.name} 尚未启用`}else if(!total){status='missing';message=`${source.name} 尚未配置当前图谱结构的字段映射`}else if(invalid){status='invalid';message=`${source.name} 存在${invalid}条无效字段映射`}else if(valid!==total||set.status!=='有效'){status='pending';message=`${source.name} 的字段映射 R${set.revision} 尚未校验`}items.push({sourceId,sourceName:source.name,sourceStatus:source.status,status,revision:Number(set.revision||1),mappingCount:total,validCount:valid,lastValidatedAt:set.last_validated_at?formatTime(set.last_validated_at):'—',message})}
