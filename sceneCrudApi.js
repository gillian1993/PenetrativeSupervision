import { assertSemanticReferences, collectSemanticReferenceIssues, editableSceneStatuses, getBoundRuleRows, getCurrentSceneRow, getSceneAggregate, mapSceneRow, makeBusinessId, toJson, parseJson, validateRuleRecord, writeSceneAudit } from './sceneRuleRepo.js'
...
async function createScene(pool,payload){
  ...
  try{
    ...
    await assertSemanticReferences(connection,{ontologyId:payload.ontologyId||'ONT-PROC',objectCode:payload.objectCode||'PROC.Supplier',eventCode:payload.eventCode||'PROC.BidConfirmed',graphVersion:payload.graphVersion||'GRAPH-20260717.2'});await connection.query(`INSERT INTO scene_versions (id,scene_id,version,status,name,domain,description,object_code,object_name,event_code,event_name,risk_level,observation_value,observation_unit,ontology_id,graph_version,organization_json,object_scope_json,exception_json,evidence_json,policy_json,check_template_json) VALUES (?,?,?,'草稿',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[versionId,code,'v0.1',String(payload.name).trim(),payload.domain||'采购',payload.description||'',payload.objectCode||'PROC.Supplier',payload.objectName||'供应商',payload.eventCode||'PROC.BidConfirmed',payload.eventName||'中标确认',payload.level||'高',Number(payload.observationValue||180),payload.observationUnit||'天',payload.ontologyId||'ONT-PROC',payload.graphVersion||'GRAPH-20260717.2',toJson(payload.organizations||['中国电子云集团']),toJson({logic:'AND',items:[]}),toJson({description:'',validUntil:''}),toJson([]),toJson([]),toJson({requirements:'',materials:[],deadlineHours:48})])
...
async function updateScene(pool,id,payload){
  ...
  await assertSemanticReferences(pool,{ontologyId:payload.ontologyId??row.ontology_id,objectCode:payload.objectCode??row.object_code,eventCode:payload.eventCode??row.event_code,graphVersion:payload.graphVersion??row.graph_version})
  const [result]=await pool.query(`UPDATE scene_versions SET name=?,domain=?,description=?,object_code=?,object_name=?,event_code=?,event_name=?,risk_level=?,observation_value=?,observation_unit=?,ontology_id=?,graph_version=?,organization_json=?,object_scope_json=?,exception_json=?,evidence_json=?,policy_json=?,check_template_json=?,status='草稿',validation_json=NULL,last_trial_id=NULL,dependency_hash='',lock_version=lock_version+1,updated_by='赵明' WHERE id=? AND lock_version=?`,[payload.name??row.name,payload.domain??row.domain,payload.description??row.description,payload.objectCode??row.object_code,payload.objectName??row.object_name,payload.eventCode??row.event_code,payload.eventName??row.event_name,payload.level??row.risk_level,Number(payload.observationValue??row.observation_value),payload.observationUnit??row.observation_unit,payload.ontologyId??row.ontology_id,payload.graphVersion??row.graph_version,toJson(payload.organizations??parseJson(row.organization_json,[])),toJson(payload.objectScope??parseJson(row.object_scope_json,{})),toJson(payload.exceptions??parseJson(row.exception_json,{})),toJson(payload.evidence??parseJson(row.evidence_json,[])),toJson(payload.policies??parseJson(row.policy_json,[])),toJson(payload.checkTemplate??parseJson(row.check_template_json,{})),row.id,row.lock_version])
...
async function validateScene(pool,id){
  ...
  if(!row.object_code)blockers.push({field:'object',tab:'basic',message:'主对象不能为空'})
  if(!row.event_code)blockers.push({field:'event',tab:'basic',message:'目标事件不能为空'})
  if(!parseJson(row.organization_json,[]).length)blockers.push({field:'organizations',tab:'scope',message:'至少选择一个适用组织'})
  if(!parseJson(row.evidence_json,[]).length)blockers.push({field:'evidence',tab:'evidence',message:'至少选择一种必备证据'})
  if(!parseJson(row.policy_json,[]).length)blockers.push({field:'policies',tab:'evidence',message:'至少选择一项制度依据'})
  ...
  const sceneRefs=await collectSemanticReferenceIssues(pool,{ontologyId:row.ontology_id,objectCode:row.object_code,eventCode:row.event_code,graphVersion:row.graph_version})
  blockers.push(...sceneRefs.blockers)
  warnings.push(...sceneRefs.warnings)
  ...
  for(const rule of enabledRules){
    const refs=await collectSemanticReferenceIssues(pool,{ontologyId:rule.ontology_id,objectCode:rule.object_code,eventCode:rule.event_code,graphVersion:rule.graph_version},'conditions')
...
export async function handleSceneCrudApi(req,res,url,{pool,sendJson,readBody}){
  ...
  if(item){const id=decodeURIComponent(item[1]);if(req.method==='GET'){const result=await getSceneAggregate(pool,id);sendJson(res,result?200:404,result||{message:'场景不存在'});return true}if(req.method==='PUT'){sendJson(res,200,await updateScene(pool,id,await readBody(req)));return true}}
  return false
}
